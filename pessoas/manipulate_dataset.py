...
def manipulate_dataset(feature_file,  image_path=None, img_ID=None, dataset_path=None, 
                    model_name="mobilefacenet", preprocessing_method="sphereface", crop_size=(96,112), gpu=False):
    """
    ...
    if dataset_path is not None:
        dataset = GenericDataLoader(dataset_path, preprocessing_method='sphereface', crop_size=(96, 112))
        dataset_dataloader = torch.utils.data.DataLoader(dataset, batch_size=8,
    ...
    if image_dataloader is not None:
        # TODO: validar o cropped_image quando o features_file nao existe
        feature = extract_features_from_image(model_name, image_dataloader, image_path, gpu=gpu)
    elif dataset_dataloader is not None:
        feature = extract_features(model_name, dataset_dataloader, gpu=gpu)
    else:
...
if __name__ == '__main__':
    manipulate_dataset("features/featurestestando123.mat", dataset_path="datasets/LFW/images")

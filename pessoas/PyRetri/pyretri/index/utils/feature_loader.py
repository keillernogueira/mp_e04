...
class FeatureLoader:
    ...
    def load_features(self, features) -> (np.ndarray, Dict, Dict):
        ...
        st_idx = 0
        for i in range(len(feature_dict['feature'][0])):
            stacked_feature.append(feature_dict['feature'][0][i])
            if(feature_dict['name'][0][i] != None):
                info_dicts[feature_dict['image'][0][i].strip()] = [feature_dict['name'][0][i].strip(), feature_dict['bbs'][0][i],[]]
            else:
                info_dicts[feature_dict['image'][0][i].strip()] = ['Pessoa não identificada', feature_dict['bbs'][0][i],[]]
        stacked_feature = np.array(stacked_feature)
        

        return stacked_feature, info_dicts
